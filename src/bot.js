...
bot.action(/^outcome:(\d+):(win|lose)$/, async (ctx) => {
  ...
  if (resolution === "dispute") {
    ...
    if (oracleResult === -1) {
      await ctx.reply("AI Oracle resolved the dispute automatically.");
      return;
    }
    if (oracleResult === -2) {
      await ctx.reply("AI Oracle found the winner, but payout could not be sent yet. We will keep you updated.");
      return;
    }
    await ctx.reply("AI Oracle is checking evidence now. If confidence stays low, community jurors will vote.");
    return;
  }
...
bot.catch(async (error, ctx) => {
  ...
  logger.error(`[BOT] Handler failed for update ${updateId}: ${message}`);
...
if (process.env.DISABLE_BOT_LAUNCH !== "1") {
  ...
  process.once("SIGTERM", async () => {
    await notifyDevInfo("⛔ TON Consensus bot received SIGTERM and is shutting down");
    bot.stop("SIGTERM");
  });

  setTimeout(() => {
    runAndNotifySelfTest(bot, "startup").catch(() => {});
  }, 15000);

  setInterval(() => {
    runAndNotifySelfTest(bot, "scheduled").catch(() => {});
  }, 6 * 60 * 60 * 1000);
}
